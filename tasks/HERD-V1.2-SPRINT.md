...
**Status:** PRs #1–#5.1, #6a, #6b, #6c, #7, and #8 landed — **v1.2 foundation complete** (last reconciled with implementation: 2026-06-13)
...
| #8 | Integration test + smoke | `tests/fleet_routing.rs` — in-process gateway (the two public handlers `heartbeat` + `chat_completions` on a hand-built `AppState`, in-memory `NodeDb`, ephemeral port — not `server::run`) proves the WHOLE chain over real HTTP: heartbeat → `NodeRegistry` → `AgentPoolSync::reconcile` → `BackendPool` → router → proxy → stub upstream. 3 tests: (1) happy path 200 + stub actually receives the routed request; (2) drain → real **503** after the agent goes stale via an injected clock (advance past TTL, **not** sleep) + reconcile; (3) model routing — request for model X lands on the agent holding X, never the other. Minimal `#[doc(hidden)] pub` test seam (`with_clock`/`Clock`, `reconcile`, `open_in_memory`) so the external test crate can drive determinism. Two-box network reachability stays manual (below). | ✅ |
...
- [x] Both modes can run on the same host (CITADEL self-test scenario) *(PR #4: guarded self-test in `tests/agent_daemon.rs` + manual smoke verified 2026-06-09; PR #8: `tests/fleet_routing.rs` proves the routed end-to-end chain in one process — heartbeat through to the agent's stub upstream, 200 with the stub actually hit. The remaining cross-machine network-reachability proof is documented manual acceptance, not automatable in-process.)*
...
- [x] Tests: `NodeRegistry` unit tests *(PR #2: 10 tests)*, heartbeat protocol tests *(PR #3: 8 tests, verified green 2026-06-05)*, daemon unit + heartbeat-client integration tests *(PR #4: 39 unit tests + `tests/agent_daemon.rs`)*, `AgentPoolSync` reconciler tests *(PR #7: 4 tests — fresh→routable, drain→503, prefix-ownership, enrolled+agent coexist)*, integration test with gateway + 1 agent in same process *(PR #8: `tests/fleet_routing.rs`, 3 tests — happy-path routed 200 with stub-received assertion, drain→503 via injected clock, model routing to the holder)*
